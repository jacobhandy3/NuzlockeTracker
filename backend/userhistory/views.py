...
class HistoryCreate(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        region = g.region
        start = Team.objects.filter(trainer=self.user).order_by('received','captured').values_list('received',flat=True)[0]
        title = str(request.user.username) + "'s Adventure in " + str(region) + " beginning on " + start.strftime('%m-%d-%Y')
        body = self.build_body(self,game_id)
        slug = slugify(title)

        #save the instance with created parameters
        serializer.save(author=request.user,game=g.name,title=title,start_date=start,body=body,slug=slug)
        #delete the team entries as we dont need that information anymore
        self.del_team(self,game_id)
    ...
    def limit_ten(self):
        ...
        if History.objects.filter(author=self.user).count()>=10:
            #delete the earliest created run
            ripObj = History.objects.filter(user=self.author).order_by('end_date')[0]
            ripObj.delete()
    ...
    def del_team(self, gameID):
        ...
        Team.objects.filter(trainer=self.user).filter(origin=gameID.id).delete()